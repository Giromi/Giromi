...
public:
    ...
    public:
        const char* what(void) const noexcept { return "Not enough minerals!"; };
    };

    class NotChoiceRace : public std::exception {
    public:
        const char* what(void) const noexcept { return "Not yet choice race"; };
    };
...
protected:
    void isPossibleCreateUnit(const unsigned int cost) {
        if (mineral < cost) {
            std::cout << race << "/" << mineral << std::endl;
            throw NotEnoughMinerals();
        }
    };
